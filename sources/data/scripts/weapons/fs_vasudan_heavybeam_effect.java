...
public class fs_vasudan_heavybeam_effect implements BeamEffectPlugin {
    ...
    public void advance(float amount, CombatEngineAPI engine, BeamAPI beam) {
        ...
        if (firing) {
            if (beam.getBrightness() < level) {
                ...
                if (Global.getCombatEngine().getTotalElapsedTime(false) - sinceLast > 0.3f) {
                    ...
                    Global.getSoundPlayer().playSound("beam_down", 1.0f, 1.0f, origin2, new Vector2f());
                }
            }
        } else {
            if (beam.getBrightness() > level) {
                ...
                if (Global.getCombatEngine().getTotalElapsedTime(false) - sinceLast > 0.3f) {
                    ...
                    Global.getSoundPlayer().playSound("bvas_loop", 1f, 1.5f, origin, new Vector2f());
                }

                sinceLast = Global.getCombatEngine().getTotalElapsedTime(false);
                // Make the beam source glow during firing
                Global.getCombatEngine().addHitParticle(origin, ZERO, 200f, 5f, 0.2f, COLOR1);
                Global.getCombatEngine().addHitParticle(origin, ZERO, 150f, 5f, 0.2f, COLOR2);
                Global.getCombatEngine().addHitParticle(origin, ZERO, 75f, 10f, 0.3f, COLOR3);
            }
        }
        ...
        if (beam.getBrightness() < level) {
            Global.getCombatEngine().addHitParticle(origin, ZERO, 200f, 5f * level, 0.2f, COLOR1);
            Global.getCombatEngine().addHitParticle(origin, ZERO, 150f, 5f * level, 0.2f, COLOR2);
            Global.getCombatEngine().addHitParticle(origin, ZERO, 75f, 10f * level, 0.3f, COLOR3);
        } else {
            Global.getCombatEngine().addHitParticle(origin, ZERO, (float) Math.random() * 100f + 100f, 5f, 0.2f, COLOR1);
            Global.getCombatEngine().addHitParticle(origin, ZERO, (float) Math.random() * 75f + 75f, 5f, 0.2f, COLOR2);
            Global.getCombatEngine().addHitParticle(origin, ZERO, (float) Math.random() * 37f + 37f, 10f, 0.3f, COLOR3);
        }
        ...
        if (interval.intervalElapsed()) {
            ...
            if (beam.getDamageTarget() != null) {
                Global.getCombatEngine().spawnExplosion(beam.getTo(), new Vector2f(), new Color(MathUtils.getRandomNumberInRange(225, 255),
                        ...
                        (float) Math.random() * 70f + 70f, 0.75f);
            }
        }
    }
}